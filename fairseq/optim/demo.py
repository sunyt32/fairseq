...
@dataclass
class FairseqDeMoConfig(FairseqDataclass):
    compression_decay: float = field(
        default=0.999, metadata={"help": "compression decay factor"}
    )
    compression_topk: int = field(
        default=32, metadata={"help": "topk sparsity"}
    )
    compression_chunk: int = field(
        default=64, metadata={"help": "chunk size"}
    )
...
@register_optimizer("demo", dataclass=FairseqDeMoConfig)
class FairseqDeMo(FairseqOptimizer):
    def __init__(self, cfg: DictConfig, params):
        super().__init__(cfg)
        self._optimizer = DeMo(
            params,
            lr=cfg.lr[0],
            compression_decay=cfg.compression_decay,
            compression_topk=cfg.compression_topk,
            compression_chunk=cfg.compression_chunk,
    ...
    def optimizer_config(self):
        ...
        return {
            ...
            "compression_decay": self.cfg.compression_decay,
            "compression_topk": self.cfg.compression_topk,
            "compression_chunk": self.cfg.compression_chunk,
