...
@dataclass
class FairseqSOAPConfig(FairseqDataclass):
    ...
    soap_eps: float = field(
        default=1e-8, metadata={"help": "epsilon for SOAP optimizer"}
    )
    optim_update_freq: int = field(
        default=1, metadata={"help": "update soap matrix every N steps"}
    )
...
@register_optimizer("soap", dataclass=FairseqSOAPConfig)
class FairseqSOAP(FairseqOptimizer):
    def __init__(self, cfg: DictConfig, params):
        super().__init__(cfg)
        self.adam_betas = eval(cfg.adam_betas)
        self._optimizer = SOAP(
            params,
            lr=cfg.lr[0],
            ...
            max_precond_dim=cfg.optim_max_preconditioner_dim,
        )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # Projecting gradients to the eigenbases of SOAP's preconditioner 
                ...
                # Projecting the exponential moving average of gradients to the eigenbases of SOAP's preconditioner 
                # i.e. projecting to the eigenbases of matrices in state['GG']
